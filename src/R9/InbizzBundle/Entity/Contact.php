...
class Contact
{
   ...
   * @ORM\PreUpdate()
   */
    public function preProjetlist()
    {
        // liste des projets du contact sou forme de string pour faciliter la recherche
        // defini depuit la jointure avec l'entity Contactprojet
        // mis a jour sur l'event PrePersist et PreUpdate
        if ($this->getTypecontact()=='allie') {
            $projetlist = ',';
            $projets = $this->getProjets();
            if ($projets !== null){
                foreach ($this->getProjets() as $projet) {
                  $projetlist .= $projet->getNom().',';
                }
            }
            $this->setProjetlist($projetlist);
        }
    }
     ...
     * Set projetlist
     *
     * @param string $projetlist
     *
     * @return Contact
     */
    public function setProjetlist($projetlist)
    {
