...
$(document).ready(function () {
    ...
    $('.actCreateContact').on('click', function () {
        ...
        if (oForm.find('.actMarque').val() != '') {
            document.location.href=oForm.attr('action')+'/'+oForm.find('.actMarque').val();
        }
    });
    ...
    var autocompleteurl = $('.searchform').attr('action').replace(/\/recherche/gi,'/autocomplete');
    $( "#searchfield" )
...
//    $('.searchform .autocompletefield').append('<div class="autocomplete"></div>');
//    var autocomplete;
//    $('.searchform .searchfield').on('change, keyup', function () {
//        var completeList = $('.searchform .autocomplete');
//        completeList.empty();
//        var sq = $('.searchform .searchfield').val();
//        
//        if (sq.lastIndexOf(',')>-1){
//            sq = sq.substr(sq.lastIndexOf(',')+1).trim();
//        }
//        
//        if (sq != '' && sq.length>1) {
//            autocomplete = $.ajax({
//                method: "GET",
//                url: $('.searchform').attr('action').replace(/\/recherche/gi,'/autocomplete?q='+encodeURI(sq))
//            })
//            .done(function (msg) {
//                var oReturn = msg;
//                oReturn.sort();
//                oReturn.forEach(function(entry) {
//                    completeList.append('<div class="autocompleteitem">'+entry+'</div>');
//                });
//            });
//        }
//    });
//    $('.searchform .autocomplete').on('click','.autocompleteitem',function(){
//        var sq = $('.searchform .searchfield').val();
//        
//        if (sq.lastIndexOf(',')>-1){
//            sq = sq.substr(0,sq.lastIndexOf(','));
//            sq += ', '+$(this).text()+', ';
//        } else {
//            sq = $(this).text()+', ';
//        }
//        
//        $('.searchform .searchfield').val(sq).focus();
        ...
        oProfilDetail.find('input[type=text]').each(function () {
            if ($(this).attr('id')!='user_tel') {
                ...
                for (var a in btnArray) {
                    if (btnArray[a].replace(/ /gi,' ')!='') $(this).before('<div class="btnpill actsearchfill">' + btnArray[a] + '</div>');
                }
            } else {
    ...
    if (oProfilDetail.length) {
        ...
        $('.profildetail.proprietaire .tools .actvalidprofil').on('click', function () {
            ...
            var formData = new FormData(),
                fields = $(".profildetail").closest('form').serializeArray();
            $.each( fields, function( i, field ) {
                formData.append(field.name, field.value);
            });
            formData.append('file', document.getElementById('user_image_file').files[0]);
            
            $('.profildetail input').removeClass('error');
            
            $.ajax({
                    //method: "POST",
                    //url: $('.marquedetail form').attr('action'),
                    //data: $('.marquedetail form').serialize()
                    url: $(".profildetail").closest('form').attr('action'),
                    type: 'POST',
...
//                })
        ...
        //upload des images
        oProfilDetail.find('#user_image_file').on('change', function () {
    ...
    if (oMarqueDetail.length) {
        ...
        //upload des images
        oMarqueDetail.find('#marque_image_file').on('change', function () {
            var fileList = this.files
            for (var i = 0; i < fileList.length; i++) {
                var file = fileList[i];
                var imageType = /image.*/;
    ...
    $('.page-footer-wrapper .actvalider').on('click', function () {
        ...
        if (oMarque.length) {
            ...
            //controle de saisi client pour création
            $('.marquedetail form input').removeClass('error');
            $('.contactitem form input').removeClass('error');
            
            if (bModeCreation) {
                var valida = true;
                var obj = $('#marque_nom');
                var checkit = obj.val()!='';
                if (!checkit) obj.addClass('error');
                valida = valida && checkit;
                
                $('.contactitem').each(function(){
                    obj = $(this).find('input[id=contact_acheteur_nom]');
                    obj = (obj.length)?obj:$(this).find('input[id=contact_marque_nom]');
                    checkit = obj.val()!='';
                    if (!checkit) obj.addClass('error');
                    valida = valida && checkit;
                    obj = $(this).find('input[id=contact_acheteur_prenom]');
                    obj = (obj.length)?obj:$(this).find('input[id=contact_marque_prenom]');
                    checkit = obj.val()!='';
                    if (!checkit) obj.addClass('error');
                    valida = valida && checkit;
                });
            ...
            // pour l upload file
        //    var oForm = $(".marquedetail form")[0];
            var formData = new FormData(),
                fields = $(".marquedetail form").serializeArray();
            $.each( fields, function( i, field ) {
                formData.append(field.name, field.value);
            });
            formData.append('file', document.getElementById('marque_image_file').files[0]);
            ...
            $.ajax({
                ...
                .done(function (msg) {
                    ...
                    if (oReturn.error==''){
                        ...
                        if (bModeCreation) {
                            fnSendAllContact(oReturn.marqueId);
                        }
                    }else{
                        for(var field in oReturn.fields){
                            $('input#marque_'+oReturn.fields[field]).addClass('error');
                        }
    ...
    if (oMarqueContact.length) {
        ...
        oMarqueContact.find('.infocontact input[type=checkbox]:checked').each(function () {
            ...
            if (btnText.replace(/ /gi,' ')!='') $(this).before('<div class="btnpill actsearchfill2">' + btnText + '</div>');
        });
    ...
    $('section.marquecontact').on('click', '.actaddcontact', function () {
        ...
        if (bCheckForm) {
            ...
            if (bModeCreation) {
                ...
                oContactItem.find('form input').removeClass('error');
                //controle de saisie client en mode creation
                var valida = true;
                var obj = null;
                var checkit = false;
                
                obj = oContactItem.find('input[id=contact_acheteur_nom]');
                obj = (obj.length)?obj:oContactItem.find('input[id=contact_marque_nom]');
                checkit = obj.val()!='';
                if (!checkit) obj.addClass('error');
                valida = valida && checkit;
                obj = oContactItem.find('input[id=contact_acheteur_prenom]');
                obj = (obj.length)?obj:oContactItem.find('input[id=contact_marque_prenom]');
                checkit = obj.val()!='';
                if (!checkit) obj.addClass('error');
                valida = valida && checkit;
                ...
                if (isContactAllie) {
                    oContactItem.removeClass('contactmodele').addClass('contactitem proprietaire');
                    oContactItem.find('.icovalidbl').css({
                        'display': 'none'
                    });
                ...
                $.ajax({
                    ...
                    .done(function (msg) {
						...
						if (oReturn.error==''){
                            ...
                            if (isContactAllie) {
                                oContactItem.html(msg.html);
                                oContactItem.removeClass('contactmodele').addClass('contactitem proprietaire'); oContactItem.find('form').attr('action',oContactItem.find('form').attr('action')+'/'+msg.contactId);
                                oContactItem.find('input[type=text]').each(function () {
                                    $(this).attr('data-reset', $(this).val());
                                });
                                oContactItem.find('select').each(function () {
                                ...
                                var oContactClone = $(msg.html);
                            //    var oContactClone = oContactItem.clone(true, false);
                            //    oContactClone.removeClass('contactmodele').addClass('contactitem proprietaire');
                            //    oContactClone.find('select').prop('selectedIndex', oContactItem.find('select').prop('selectedIndex'));
                                oContactClone.find('form').attr('action',oContactClone.find('form').attr('action')+'/'+msg.contactId);
                                oContactClone.find('input[type=text]').each(function () {
                                    $(this).attr('data-reset', $(this).val());
                                });
                                oContactClone.find('select').each(function () {
							...
							for(var field in oReturn.fields){
                                oContactItem.find('input#contact_acheteur_'+oReturn.fields[field]).addClass('error');
                                oContactItem.find('input#contact_marque_'+oReturn.fields[field]).addClass('error');
                            }
    ...
    $('section.marquecontact').on('click', '.actdelcontact', function () {
        ...
        if (confirm("Supprimer le contact ?")) {
            ...
            if (bModeCreation) {
                ...
                //is contact allie
                isContactAllie = ($(this).closest('.marquecontactallie').length) ? 1 : 0;
				var formaction = oContactItem.find('form').attr('action');
				oContactItem.find('form').attr('action',formaction.replace(/\/update\//gi,'/delete/'));
                    ...
                    .done(function (msg) {
						...
						if (oReturn.error==''){
							...
							if (isContactAllie) {
								...
								oContactItem.find('form').attr('action',oReturn.urlallieaction);
							} else {
								setTimeout(function () {
									oContactItem.remove()
								}, 3000);
...
function fnSendAllContact(marqueId) {
    ...
    if (oContactForms.length) {
        if (oContactForms.length > contactSendIndex) {
            ...
            $.ajax({
                    method: "POST",
                    url: oContactItem.find('form').attr('action')+'/'+marqueId,
                    data: oContactItem.find('form').serialize()
                })
                .done(function (msg) {
                    oContactItem.removeClass('submitatt submitok submitfail').addClass('submitok');
            ...
            console.log('fini');
            document.location.href+='/'+marqueId;
